    ...
    def run(self, subnet, mask,
        ...
        if kwargs['master_subnet'] is not None:
            ...
            subnetlist = (subnets_api.list_subnets_cidr(
                subnet_cidr=msub))['data']
            sub = [x for x in subnetlist if x['sectionId'] == sect[0]['id']]
            lib.utils.check_list(
                t_list=sub, t_item=msub, t_string='master subnet')
        ...
        if kwargs['vlan'] is not None:
            ...
            l2domains_api = lib.phpipam.controllers.L2DomainsApi(
                phpipam=ipam)
            tools_vlans_api = lib.phpipam.controllers.ToolsVlansApi(
                phpipam=ipam)
